  ...
  if( millis() - previousMillis >100){
    ...
    // si stop
      ...
      // si changement de sens
        ...
        if ((abs(cmdR)-abs(prevCmdR))>maxAcc * 2){
          cmdR = prevCmdR+ maxAcc * 2 * (cmdR-prevCmdR)/ abs(cmdR-prevCmdR);
        }
        if ((abs(cmdL)-abs(prevCmdL))>maxAcc * 2){
          cmdL = prevCmdL+ maxAcc * 2 * (cmdL-prevCmdL)/ abs(cmdL-prevCmdL);
        }
        prevCmdR = cmdR;
        prevCmdL = cmdL;
